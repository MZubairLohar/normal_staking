...
contract BLldToBldStake is StakingTokenWrapper, RewardsDistributionRecipient{
    ...
    uint256 internal rewardPercent = 2; // 200%
    uint256 internal blocksPerYear = 2102400; // 4 blocks / min
    // uint256 internal rewardPercentPerBlock = (rewardPercent / blocksPerYear) * 1e24; // 951293759512937500 %
...
// ********************************************
        ...
        isAccount(_beneficiary)
    {
        ...
        stkIn.reward = RewardCalculator(_beneficiary);
    ...
    function claimReward() internal isAccount(msg.sender){
        uint reward = RewardCalculator(msg.sender);
        // Staker storage stk = stakerInfo[msg.sender];
    ...
    function RewardCalculator(address _user) public view returns (uint256) {
        Staker storage stk = stakerInfo[_user];
        
        uint256 blocks = block.timestamp - stk.stakeStarted;
        uint256 reward = (blocks * (rewardPercent / blocksPerYear) * stk.userStakedTokens * 100) / 100;
        return reward;
